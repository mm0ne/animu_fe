...
export default function AnimeThumbnail({image_link, mal_link ,detail, genres, aliases}: AnimeThumbnailProps){
    ...
    return(
        <section className="flex flex-col justify-start gap-y-3 w-full lg:max-w-[15em] px-2 py-4 lg:py-0">
            ...
            <Link target="_blank" href={mal_link} className="text-white w-full self-center lg:w-full flex justify-center items-center font-semibold gap-x-1 bg-blue-600 py-1 rounded-md hover:text-yellow-500 transition-all duration-100">
            ...
            <article className="font-semibold max-w-fit text-[15px] md:text-[18px]">
                <p className="">Year : <span className="font-normal">{detail.year}</span></p>
                <p className="">Genre : <span className="font-normal">{genre.join(", ")}</span></p>
                <p className="">Seasons : <span className="font-normal">{detail.seasons}</span></p>
                <p className="">Alias : <span className="font-normal">{alias.join(",")}</span></p>
            </article>
        </section>
    )
}